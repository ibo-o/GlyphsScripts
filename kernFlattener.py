...
# This script is written by Rainer Erich Scheichelbauer https://github.com/mekkablue/Glyphs-Scripts/blob/master/Kerning/Kern%20Flattener.py
# 
...
for i in range(len(thisFont.instances) - 1, -1, -1):
  ...
  if thisInstance.type == INSTANCETYPESINGLE:
    ...
    featuresToRemove = ["kern", "mark", "mkmk", "cpsp"]

    if currentRemoveFeatures:
        # Convert tuple → list before concatenation
        currentRemoveFeatures = list(currentRemoveFeatures)
        thisInstance.customParameters["Remove Features"] = list(
            set(currentRemoveFeatures + featuresToRemove)
        )
    else:
        thisInstance.customParameters["Remove Features"] = featuresToRemove
